 ...
 * @file     main.c
 * @brief
 *           Configure SPI0 as master mode and demonstrate how to communicate
 *           with an off-chip SPI Slave device with PDMA. This sample
 *           code needs to work with SPI_SLAVE_WITH_PDMA sample code.
 *
...
volatile uint8_t u8SrcArray[TEST_COUNT];
volatile uint8_t u8DestArray[TEST_COUNT];
...
int main(void)
{
    ...
    printf("\n\n");
    printf("+--------------------------------------------------------------+\n");
    printf("|                  SPI Master with PDMA Sample Code            |\n");
    printf("+--------------------------------------------------------------+\n");
    printf("\n");
    ...
    {
        u8SrcArray[u32DataCount] = u32DataCount;
        u8DestArray[u32DataCount] = 0;
    }

    /* Set PA15 in input mode*/
    ...
    PDMA_SetTransferAddr(DMA_SPI1_TX, (uint32_t)u8SrcArray, PDMA_SAR_INC, (uint32_t)&SPI1->TX[0], PDMA_DAR_FIX);
    ...
    PDMA_SetTransferAddr(DMA_SPI1_RX, (uint32_t)&SPI1->RX[0], PDMA_SAR_FIX, (uint32_t)u8DestArray, PDMA_DAR_INC);
    ...
    /* Wait fot PDMA RX finish */
    while (u32RXFinFlag == 0) ;

    for (u32DataCount = 0; u32DataCount < TEST_COUNT; u32DataCount++)
    {
        if (u8SrcArray[u32DataCount] != u8DestArray[u32DataCount])
            u32ErrCount++;
    }

    printf("\nSPI1 Master with PDMA test ");
